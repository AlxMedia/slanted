...
/* ------------------------------------ */
	private function _create_tabs($tabs,$count) {
		...
		$titles = array(
			'recent'	=> __('Recent Posts','slanted'),
			'popular'	=> __('Popular Posts','slanted'),
			'comments'	=> __('Recent Comments','slanted'),
			'tags'		=> __('Tags','slanted')
		);
		$icons = array(
			...
			<ul id="tab-comments" class="alx-tab group <?php if($instance['comments_avatars']) { echo 'avatars-enabled'; } ?>">
				...
				<li>
						...
						<div class="tab-item-inner group">
							...
							<div class="tab-item-name"><?php echo esc_attr( $comment->comment_author ); ?> <?php _e('says:','slanted'); ?></div>
