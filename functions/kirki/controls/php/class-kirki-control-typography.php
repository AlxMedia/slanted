		...
		<div class="wrapper">
			...
			<# if ( data.default['font-family'] ) { #>
				...
				<div class="font-family">
					<h5><?php esc_attr_e( 'Font Family', 'slanted' ); ?></h5>
					<select {{{ data.inputAttrs }}} id="kirki-typography-font-family-{{{ data.id }}}" placeholder="<?php esc_attr_e( 'Select Font Family', 'slanted' ); ?>"></select>
				</div>
				<# if ( ! _.isUndefined( data.choices['font-backup'] ) && true === data.choices['font-backup'] ) { #>
					<div class="font-backup hide-on-standard-fonts kirki-font-backup-wrapper">
						<h5><?php esc_attr_e( 'Backup Font', 'slanted' ); ?></h5>
						<select {{{ data.inputAttrs }}} id="kirki-typography-font-backup-{{{ data.id }}}" placeholder="<?php esc_attr_e( 'Select Font Family', 'slanted' ); ?>"></select>
					</div>
				<# } #>
				<# if ( true === data.show_variants || false !== data.default.variant ) { #>
					<div class="variant kirki-variant-wrapper">
						<h5><?php esc_attr_e( 'Variant', 'slanted' ); ?></h5>
				...
				<# if ( true === data.show_subsets ) { #>
					<div class="subsets hide-on-standard-fonts kirki-subsets-wrapper">
						<h5><?php esc_attr_e( 'Subset(s)', 'slanted' ); ?></h5>
			...
			<# if ( data.default['font-size'] ) { #>
				...
				<div class="font-size">
					<h5><?php esc_attr_e( 'Font Size', 'slanted' ); ?></h5>
			...
			<# if ( data.default['line-height'] ) { #>
				...
				<div class="line-height">
					<h5><?php esc_attr_e( 'Line Height', 'slanted' ); ?></h5>
			...
			<# if ( data.default['letter-spacing'] ) { #>
				...
				<div class="letter-spacing">
					<h5><?php esc_attr_e( 'Letter Spacing', 'slanted' ); ?></h5>
			...
			<# if ( data.default['word-spacing'] ) { #>
				...
				<div class="word-spacing">
					<h5><?php esc_attr_e( 'Word Spacing', 'slanted' ); ?></h5>
			...
			<# if ( data.default['text-align'] ) { #>
				...
				<div class="text-align">
					<h5><?php esc_attr_e( 'Text Align', 'slanted' ); ?></h5>
					<div class="text-align-choices">
						<input {{{ data.inputAttrs }}} type="radio" value="inherit" name="_customize-typography-text-align-radio-{{ data.id }}" id="{{ data.id }}-text-align-inherit" <# if ( data.value['text-align'] === 'inherit' ) { #> checked="checked"<# } #>>
							<label for="{{ data.id }}-text-align-inherit">
								...
								<span class="screen-reader-text"><?php esc_attr_e( 'Inherit', 'slanted' ); ?></span>
							</label>
						</input>
						<input {{{ data.inputAttrs }}} type="radio" value="left" name="_customize-typography-text-align-radio-{{ data.id }}" id="{{ data.id }}-text-align-left" <# if ( data.value['text-align'] === 'left' ) { #> checked="checked"<# } #>>
							<label for="{{ data.id }}-text-align-left">
								...
								<span class="screen-reader-text"><?php esc_attr_e( 'Left', 'slanted' ); ?></span>
							</label>
						</input>
						<input {{{ data.inputAttrs }}} type="radio" value="center" name="_customize-typography-text-align-radio-{{ data.id }}" id="{{ data.id }}-text-align-center" <# if ( data.value['text-align'] === 'center' ) { #> checked="checked"<# } #>>
							<label for="{{ data.id }}-text-align-center">
								...
								<span class="screen-reader-text"><?php esc_attr_e( 'Center', 'slanted' ); ?></span>
							</label>
						</input>
						<input {{{ data.inputAttrs }}} type="radio" value="right" name="_customize-typography-text-align-radio-{{ data.id }}" id="{{ data.id }}-text-align-right" <# if ( data.value['text-align'] === 'right' ) { #> checked="checked"<# } #>>
							<label for="{{ data.id }}-text-align-right">
								...
								<span class="screen-reader-text"><?php esc_attr_e( 'Right', 'slanted' ); ?></span>
							</label>
						</input>
						<input {{{ data.inputAttrs }}} type="radio" value="justify" name="_customize-typography-text-align-radio-{{ data.id }}" id="{{ data.id }}-text-align-justify" <# if ( data.value['text-align'] === 'justify' ) { #> checked="checked"<# } #>>
							<label for="{{ data.id }}-text-align-justify">
								...
								<span class="screen-reader-text"><?php esc_attr_e( 'Justify', 'slanted' ); ?></span>
							</label>
						</input>
					</div>
				</div>
			...
			<# if ( data.default['text-transform'] ) { #>
				...
				<div class="text-transform">
					<h5><?php esc_attr_e( 'Text Transform', 'slanted' ); ?></h5>
					<select {{{ data.inputAttrs }}} id="kirki-typography-text-transform-{{{ data.id }}}">
						<option value="none"<# if ( 'none' === data.value['text-transform'] ) { #>selected<# } #>><?php esc_attr_e( 'None', 'slanted' ); ?></option>
						<option value="capitalize"<# if ( 'capitalize' === data.value['text-transform'] ) { #>selected<# } #>><?php esc_attr_e( 'Capitalize', 'slanted' ); ?></option>
						<option value="uppercase"<# if ( 'uppercase' === data.value['text-transform'] ) { #>selected<# } #>><?php esc_attr_e( 'Uppercase', 'slanted' ); ?></option>
						<option value="lowercase"<# if ( 'lowercase' === data.value['text-transform'] ) { #>selected<# } #>><?php esc_attr_e( 'Lowercase', 'slanted' ); ?></option>
						<option value="initial"<# if ( 'initial' === data.value['text-transform'] ) { #>selected<# } #>><?php esc_attr_e( 'Initial', 'slanted' ); ?></option>
						<option value="inherit"<# if ( 'inherit' === data.value['text-transform'] ) { #>selected<# } #>><?php esc_attr_e( 'Inherit', 'slanted' ); ?></option>
					</select>
				</div>
			<# } #>
			...
			<# if ( data.default['text-decoration'] ) { #>
				...
				<div class="text-decoration">
					<h5><?php esc_attr_e( 'Text Decoration', 'slanted' ); ?></h5>
					<select {{{ data.inputAttrs }}} id="kirki-typography-text-decoration-{{{ data.id }}}">
						<option value="none"<# if ( 'none' === data.value['text-decoration'] ) { #>selected<# } #>><?php esc_attr_e( 'None', 'slanted' ); ?></option>
						<option value="underline"<# if ( 'underline' === data.value['text-decoration'] ) { #>selected<# } #>><?php esc_attr_e( 'Underline', 'slanted' ); ?></option>
						<option value="overline"<# if ( 'overline' === data.value['text-decoration'] ) { #>selected<# } #>><?php esc_attr_e( 'Overline', 'slanted' ); ?></option>
						<option value="line-through"<# if ( 'line-through' === data.value['text-decoration'] ) { #>selected<# } #>><?php esc_attr_e( 'Line-Through', 'slanted' ); ?></option>
						<option value="initial"<# if ( 'initial' === data.value['text-decoration'] ) { #>selected<# } #>><?php esc_attr_e( 'Initial', 'slanted' ); ?></option>
						<option value="inherit"<# if ( 'inherit' === data.value['text-decoration'] ) { #>selected<# } #>><?php esc_attr_e( 'Inherit', 'slanted' ); ?></option>
					</select>
				</div>
			<# } #>
			...
			<# if ( data.default['margin-top'] ) { #>
				...
				<div class="margin-top">
					<h5><?php esc_attr_e( 'Margin Top', 'slanted' ); ?></h5>
			...
			<# if ( data.default['margin-bottom'] ) { #>
				...
				<div class="margin-bottom">
					<h5><?php esc_attr_e( 'Margin Bottom', 'slanted' ); ?></h5>
			...
			<# if ( false !== data.default['color'] && data.default['color'] ) { #>
				...
				<div class="color">
					<h5><?php esc_attr_e( 'Color', 'slanted' ); ?></h5>
